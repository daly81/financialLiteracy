...
//GETTING QUIZ ONE SCORE FOR CURRENT USER
function get_best_score($quiz_number){
  global $res;
	$con = db_connect();
  session_start();
	$stm = $con->prepare(Query::GET_USER_SCORE);
  $stm->execute(array(
    ':user_id' => $_SESSION['user_id'],
...
//SAVING HIGH SCORE BANK BREAKER
function save_score($highScore){
  global $res;
  $con = db_connect();
  session_start();
  ...
  $oldScore = $checkScore->fetch();

  if ($highScore > $oldScore['mod_one_score']){
    $stm = $con->prepare(Query::UPDATE_BANK_BREAKER);
    $stm->execute(array(
      ':user_id' => $_SESSION['user_id'],
      ':mod_one_score' => $highScore
    ));
  }
  if(empty($oldScore)){
    $stm = $con->prepare(Query::SAVE_BANK_BREAKER);
    $stm->execute(array(
      ':user_id' => $_SESSION['user_id'],
      ':mod_one_score' => $highScore
      // ':gameOne' => $gameOne
